...
namespace Kraken.Net.Interfaces
{
    ...
    public interface IKrakenSocketClient: ISocketClient
    {
        ...
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        Task<CallResult<UpdateSubscription>> SubscribeToBalancesUpdatesAsync(string socketToken, Action<DataEvent<Dictionary<string, decimal>>> balancesHandler, Action<DataEvent<IEnumerable<KrakenLedgerEntry>>> ledgesHandler);
